...
wp-database_sync() {
    ...
    SCRIPT="gunzip ${migrationDbDumpFolderLocationRemote}/$DB_NAME.sql.gz;
    cd ${migrationDbDumpFolderLocationRemote}
    php ${serverRootRemote}/bin/wp db import $DB_NAME.sql
    php ${serverRootRemote}/bin/wp search-replace 'http://'$VIRTUAL_HOST '${domainNameProduction}' --skip-columns=guid --skip-tables=wp_users
    rm $DB_NAME.sql
    "
    select yn in "Yes" "No"; do
        case $yn in
